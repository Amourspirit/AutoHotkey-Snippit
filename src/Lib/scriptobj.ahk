...
class scriptobj
{
    ...
    Pwd                         := ""
    LaunchedBy                  := "self"
    ThisScript                  := ""       ; Placeholder fo the actual name of the scirpt without extension for example Enlaunch
    Res                         := ""       ; Instance of MfResourceSingletonBase that contains GetResource
    Profile                     := ""
    ...
    getparams() {
        ...
        if (InStr(param, "-nevc")
        ||  InStr(param, "--noevernoteversioncheck")){
            ; primary use is to pass param to know if evernote version check should be done
            ; if a script does the check and then starts a second script then the second script can ignore versoin check
            script.SkipEvernoteVersionCheck := true
        }
        if (InStr(param, "-lb")
        ||  InStr(param, "--launchedby")){
            strExpression := "(?:-lb|--launchedby)\s(\w+\.\w+)"
            RegexMatch(param,strExpression, df) ; replace sparam with -sd at the beginning
            script.LaunchedBy := df1
            df := Null
        }
        ...
        ||  InStr(param, "--save-debug")){
            strExpression := "(?:-sd|--save-debug)\s(\w+\.\w+)"
            RegexMatch(param,strExpression, df) ; replace sparam with -sd at the beginning
            sparam := "-sd " df1  a_space           ; also save the output file name next to it
            df := null
        }
        if (InStr(param, "-pwd")
        ||  InStr(param, "--password")){
			strExpression := "(?:-pwd|--password)\s([a-zA-Z0-9?!@#$%^&*()|.,'~""``]*)"
            RegexMatch(param,strExpression, df) ; replace sparam with -sd at the beginning
            ;sparam := "-pwd " df1  a_space           ; also save the password name next to it
            script.Pwd := df1
            df := null
        }
        strExpression := null
